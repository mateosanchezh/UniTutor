    ...
    public class AuthController {
        ...
        public ResponseEntity<?> login(@RequestBody Map<String, String> credentials) {
            ...
            if (passwordEncoder.matches(password, usuario.getContrasena())) {
                // No olvides quitar la contraseña antes de devolver el usuario
                usuario.setContrasena(null);

                // Generar el token usando el JwtService
                String token = jwtService.generateToken(usuario);

                // Devuelve el token y los detalles del usuario
        ...
        public ResponseEntity<?> logout(HttpServletRequest request) {
            // Aquí, en lugar de invalidar el token, simplemente se puede eliminar del lado del cliente
